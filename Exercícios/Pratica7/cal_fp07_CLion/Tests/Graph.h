...
template <class T> class Comparator;    // For Kruskal
...
class Vertex {
    ...
    Vertex<T>* parent = nullptr; // For Kruskal
...
public:
	...
	friend class Comparator<T>; // For Kruskal
...
vector<Vertex<T>* > Graph<T>::calculatePrim() {
	// TODO
    auto s = initSingleSource(vertexSet[0]->getInfo());
    MutablePriorityQueue<Vertex<T>> q;
    q.insert(s);
    while( ! q.empty() ) {
        auto v = q.extractMin();
...
class Comparator{
    public:
        int operator() (const Edge<T> &e1, const Edge<T> &e2) {
            if (e1.getWeight() == e2.getWeight())
                return (e1.orig->getInfo() > e1.dest->getInfo());
...
vector<Vertex<T>*> Graph<T>::calculateKruskal() {
	...
	int edgesAccepted = 0;
	// INCOMPLETO !!
	priority_queue<Edge<T>, vector<Edge<T>>, Comparator<T>> queue;
